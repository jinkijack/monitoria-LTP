	...
	scanf("%d",&op);
		if(op==1){
			...
			mostraMatriz(m3, linhas, cols);
				...
				printf("Não é possível realizar essa operação(Número de linhas e colunas diferentes)!");
			}
		}
		if(op==2){
			if(linhas&&linhas==linhasB&&colsB){
		...
		mostraMatriz(m3, linhasB, cols);
			...
			printf("Não é possível realizar essa operação(Número de colunas da matriz A diferente do número de linhas da matriz B)!");
		}
	}
	return 0;
}
